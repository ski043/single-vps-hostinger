...
- `echo "$GHCR_TOKEN"` prints your GitHub Container Registry token from the environment
- the `|` pipe passes that token directly into the next command
- `docker login ghcr.io` logs your local Docker client into GitHub Container Registry
- `-u "$GHCR_USERNAME"` tells Docker which GitHub account to log in as
- `--password-stdin` reads the token from standard input instead of exposing it in the shell history
...
- `docker buildx build` uses Docker Buildx, which supports modern multi-stage builds and cross-platform builds
- `--platform linux/amd64` builds for the common x86_64 Linux VPS architecture
- `--target runner` tells Docker to stop at the `runner` stage in the `Dockerfile`, which is the final Next.js runtime image
- `--build-arg DEPLOYMENT_VERSION="$IMAGE_TAG"` passes your release tag into the build
- `-t "$APP_IMAGE"` gives the image its full GHCR name and tag
- `--push` uploads the finished image straight to GHCR
- `.` means "use the current repo root as the build context"
...
- this is the same build flow as above, but for the migration container
- `--target migrator` selects the `migrator` stage from the `Dockerfile`
- that image is only used to run `prisma migrate deploy` on the VPS
- pushing it to GHCR lets the server pull the exact same migration image you built locally
...
- `docker-compose.prod.yml` tells Docker Compose which services to run on the VPS
- `Caddyfile` tells Caddy how to route traffic and handle HTTPS
- `$VPS_USER@$VPS_IP:$APP_DIR/` is the destination on the server
- only deployment files are copied here, not the full app source code
...
- `cd "$APP_DIR"` moves into the deployment directory on the VPS
- `ls -la` lists all files, including hidden ones like `.env`
- this is just a quick sanity check that the expected files are really on the server
...
export GHCR_USERNAME="ski043"
export GHCR_READ_TOKEN="token"
export APP_IMAGE="ghcr.io/ski043/hosting-vercel-local-hostinger:app-c4b5ec1"
export MIGRATION_IMAGE="ghcr.io/ski043/hosting-vercel-local-hostinger:migrate-c4b5ec1"
```

What these commands do:

- `export` creates environment variables in your current shell session
- `GHCR_USERNAME` and `GHCR_READ_TOKEN` are used to authenticate Docker against GHCR
- `APP_IMAGE` points to the published app image you want to deploy
- `MIGRATION_IMAGE` points to the published migration image you want to run once
- these variables make the later commands shorter and less error-prone
...
- `cd "$APP_DIR"` makes sure you are writing the file in the deploy folder
- `cat <<EOF ... EOF` is a heredoc, a simple way to create a multi-line file from the shell
- `> .deploy.env` writes the output into a new file named `.deploy.env`
- this file is only for deployment image tags, separate from the app's normal runtime `.env`
...
- it logs the VPS Docker client into GHCR
- this time you use the read token because the server only needs to pull images, not push them
- once logged in, `docker pull` can download your private images
...
- `docker pull` downloads the exact tagged images from GHCR onto the VPS
- pulling first makes the deployment more explicit and easier to debug
- it also confirms the image names and registry permissions are correct before Compose starts containers
...
- `docker compose` runs the services defined in `docker-compose.prod.yml`
- `--env-file .env` loads runtime app settings like domains, database credentials, and auth config
- `--env-file .deploy.env` loads the image tags you want this deploy to use
- `-f docker-compose.prod.yml` tells Compose which file to read
- `up -d` starts the containers in detached mode so they keep running in the background
- `postgres redis uptime-kuma` starts only those three services first
...
- `run migrate` starts the one-off migration container
- inside that container, the image runs `prisma migrate deploy`
- `--rm` deletes the container after it finishes so you do not leave behind stopped ops containers
- this updates the database schema before the app starts serving traffic
...
- this starts the actual Next.js app container and the Caddy reverse proxy
- `app` serves the site internally on the Docker network
- `caddy` is the public entrypoint that binds to ports `80` and `443`
- Caddy then forwards incoming web traffic to the internal app container
...
- `ps` shows the current state of the Compose services
- use it to confirm the containers are up instead of guessing
- if something exited immediately, this is usually the first place you notice it
...
- removes dangling Docker images that are no longer attached to a tag
- `-f` skips the confirmation prompt
- this is optional housekeeping to free a bit of disk space after deploys
